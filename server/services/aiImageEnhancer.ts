...
export class AIImageEnhancer {
  ...
  private async tryStabilityAPI(options: EnhancementOptions, prompt: string): Promise<AIServiceResponse> {
    try {
      ...
      console.log('🎨 Using Stability AI for enhancement...');
      ...
      formData.append('image_strength', options.isBlurred ? '0.5' : '0.35'); // More strength for blurred images
      formData.append('steps', options.quality === 'ultra' ? '50' : options.quality === 'high' ? '40' : '30');
      formData.append('seed', '0');
      formData.append('cfg_scale', '7'); // Better prompt adherence
      ...
      if (response.data.artifacts && response.data.artifacts.length > 0) {
        ...
        console.log('✅ Stability AI enhancement successful!');
      ...
      let errorMessage = 'Stability AI failed';
      if (error.response) {
        errorMessage = `Stability AI failed (${error.response.status}): ${error.response.data?.message || error.message}`;
      } else {
        errorMessage = `Stability AI failed: ${error.message}`;
      }
